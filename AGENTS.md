...
This repo (`bunkbot`) contains deployment config — **not the OpenClaw source code**. On the server, it lives at `/root/bunkbot/` alongside the OpenClaw software at `/root/openclaw/`.

```
bunkbot/                             # Deployment config (this repo)
├── install-openclaw.sh              # Main installer script
...
├── openclaw-hetzner-guide.md        # Detailed beginner guide
...
**On the server:**
```
/root/bunkbot/                       # This repo (cloned via deploy key)
/root/openclaw/                      # OpenClaw source (cloned from GitHub)
├── Dockerfile                       # Base image (upstream)
├── Dockerfile.skills                # Extended image (copied from bunkbot)
├── docker-compose.yml               # Services (copied from bunkbot)
...
### Deploying Changes
1. Edit files locally in bunkbot
2. Commit and push: `git push`
3. Pull on server: `ssh root@openclaw 'cd /root/bunkbot && git pull'`
4. If Dockerfile.skills or docker-compose.yml changed:
   ```bash
   cp /root/bunkbot/Dockerfile.skills /root/openclaw/
   cp /root/bunkbot/docker-compose.yml /root/openclaw/
   cd /root/openclaw && docker build -t openclaw:latest -f Dockerfile.skills . && docker compose up -d
   ```
5. Or re-run the full installer: `bash /root/bunkbot/install-openclaw.sh`
...
- The install script and companion files (Dockerfile.skills, docker-compose.yml, openclaw.json.template) are the source of truth for deployment
...
**Maintainer**: Olly
**Agent**: Bunk (anthropic/claude-haiku-4-5)
