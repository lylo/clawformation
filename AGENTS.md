...
This repo (`pinkubate`) contains deployment config — **not the OpenClaw source code**. On the server, it lives at `/root/pinkubate/` alongside the OpenClaw software at `/root/openclaw/`.

```
pinkubate/                           # Deployment config (this repo)
...
**On the server:**
```
/root/pinkubate/                     # This repo (cloned via deploy key)
/root/openclaw/                      # OpenClaw source (cloned from GitHub)
├── Dockerfile                       # Base image (upstream)
├── Dockerfile.skills                # Extended image (copied from pinkubate)
├── docker-compose.yml               # Services (copied from pinkubate)
...
### Deploying Changes
1. Edit files locally in pinkubate
2. Commit and push: `git push`
3. Pull on server: `ssh root@YOUR_VPS 'cd /root/pinkubate && git pull'`
4. If Dockerfile.skills or docker-compose.yml changed:
   ```bash
   cp /root/pinkubate/Dockerfile.skills /root/openclaw/
   cp /root/pinkubate/docker-compose.yml /root/openclaw/
   cd /root/openclaw && docker build -t openclaw:latest -f Dockerfile.skills . && docker compose up -d
   ```
5. Or re-run the full installer: `bash /root/pinkubate/setup.sh`
